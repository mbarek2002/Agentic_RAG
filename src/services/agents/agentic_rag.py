...
class AgenticRAGService:
    ...
    async def ask(
        ...
        try:
            if self.langfuse_tracer and self.langfuse_tracer.client:
                # v2 SDK: trace_rag_request() is a context manager yielding a
                # StatefulTraceClient (or None on failure, handled the same
                # as tracing-disabled below).
                with self.langfuse_tracer.trace_rag_request(
                    query=query,
                    user_id=user_id,
    ...
    async def _run_workflow(
        ...
        try:
            ...
            # No LangChain CallbackHandler here: Langfuse v2's callback
            # integration (langfuse.callback.CallbackHandler) is incompatible
            # with the installed LangChain 1.x (it imports langchain.schema.*
            # paths removed in 1.0). Each node instead creates its own
            # span/generation manually via LangfuseTracer.create_span()/
            # end_span(), which is what actually produces the per-node traces.
            ...
            # Update trace with error (v2 SDK: no explicit .end(), traces are
            # just updated and flushed)
            if trace:
